    ...
    class Ec2ServerCreate < Knife
      ...
      def run
        ...
        unless hashed_tags.keys.include? "Name"
          hashed_tags["Name"] = locate_config_value(:chef_node_name) || node_name(server)
        end
      ...
      def bootstrap_for_node(server,ssh_host)
        ...
        bootstrap.config[:chef_node_name] = locate_config_value(:chef_node_name) || node_name(server)
